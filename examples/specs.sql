...
partition_holds property invariant catches regressions).', 'spec', 0, '2026-05-31T04:11:47.877070+00:00', '2026-06-23T07:07:40.700381+00:00', 'design', NULL);
...
THE LITMUS (authority-vs-copy). Before typing a literal into a slice, ask: is this slice the AUTHORITY for the value, or a COPY of one that lives in code? Authority → state it, it''s a decision (D238''s "three networks, these isolation roles" is a topology decision — stays). Copy → replace the literal with the derivation rule + a pointer to the code home ("pool sized K×parallel×2, raise in lockstep; see config.py:pool_size"). The reusable move: pin the rule/formula/policy, point to code for the resolved value.
