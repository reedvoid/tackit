...
Backs: D17, D32.', 'spec', 0, '2026-05-31T04:11:47.876487+00:00', '2026-06-07T05:51:06.007644+00:00', 'schema', NULL);
...
INSERT INTO tasks (id, name, description, status, stale, created_at, updated_at, kind, wont_do_reason) VALUES (64, 'D22 — Board view (CLI, human-readable) (added 2026-05-30)', 'tackit board renders the state as a human-readable, dependency-aware board in the terminal — grouped (In Flight / Done), colored (open=accent, stale=alert, done=dim; color only on a TTY), each task showing its labels and its needs→ / unblocks→ edges. Filter MODES cover the common questions: default (everything), --status open ("what''s outstanding," the #1 ask), --label X (a group), --stale (after big changes). --json emits the structured slices. CLI-only (the agent uses the structured ls/stale/show tools); it''s the human''s at-a-glance view, the visual cousin of D16 render. (HTML export was considered and dropped — the CLI board + the agent''s change-summaries cover the need.)', 'spec', 0, '2026-05-31T04:11:47.877336+00:00', '2026-06-07T05:56:59.373206+00:00', 'design', NULL);
...
invariant).', 'spec', 0, '2026-06-01T01:52:06.009742+00:00', '2026-06-07T05:51:06.007644+00:00', 'design', NULL);
...
Deferred (triaged, revisit ONLY if lean defaults prove insufficient — not a graveyard backlog): general `fields` whitelist; pagination (limit/offset); total / count-only. Each was considered and set aside because the lean default + show(id) cover the 95% case (one body at a time = show).', 'spec', 0, '2026-06-05T08:04:29.435065+00:00', '2026-06-07T06:02:34.617971+00:00', 'design', NULL);
