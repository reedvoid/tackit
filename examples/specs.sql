...
Backs: D17, D32.', 'spec', 1, '2026-05-31T04:11:47.876487+00:00', '2026-06-23T07:03:50.511837+00:00', 'schema', NULL);
...
INSERT INTO tasks (id, name, description, status, stale, created_at, updated_at, kind, wont_do_reason) VALUES (43, 'D1 — Persistent task store', 'tackit keeps all state in a single local SQLite file (WAL mode). A project''s truth lives in one place, outside the agent''s context window, and survives across sessions and compaction.', 'spec', 1, '2026-05-31T04:11:47.876559+00:00', '2026-06-23T07:03:50.273735+00:00', 'design', NULL);
...
INSERT INTO tasks (id, name, description, status, stale, created_at, updated_at, kind, wont_do_reason) VALUES (45, 'D3 — Task create / read / update', 'Create a task (auto-assigned monotonic id, name, description, kind), read it back, and edit its name/description. **`kind` (design|schema|production|meta, D26) is REQUIRED at create time** (T94) and determines the row''s partition under D36 (kind ∈ {design,schema} ⟹ status ∈ {spec,retired}; kind ∈ {production,meta} ⟹ status ∈ {open,closed,wont_do}). The atomic unit of work; everything else attaches to it.', 'spec', 1, '2026-05-31T04:11:47.876630+00:00', '2026-06-23T07:03:50.511989+00:00', 'design', NULL);
...
partition_holds property invariant catches regressions).', 'spec', 1, '2026-05-31T04:11:47.877070+00:00', '2026-06-23T07:03:50.274332+00:00', 'design', NULL);
...
invariant).', 'spec', 1, '2026-06-01T01:52:06.009742+00:00', '2026-06-23T07:03:50.512127+00:00', 'design', NULL);
...
Deferred (triaged, revisit ONLY if lean defaults prove insufficient — not a graveyard backlog): general `fields` whitelist; pagination (limit/offset); total / count-only. Each was considered and set aside because the lean default + show(id) cover the 95% case (one body at a time = show).', 'spec', 0, '2026-06-05T08:04:29.435065+00:00', '2026-06-23T06:40:45.779024+00:00', 'design', NULL);
...
INSERT INTO description_revisions (id, task_id, prev_name, prev_description, delta, edited_at) VALUES (63, 59, 'D17 — Full-text search', 'Find tasks by keyword over name + description, ranked, via SQLite FTS5 — in-process, no external service or GPU. Returns matching task ids + titles + scores. This is the agent''s entry point for finding tasks (e.g. the ids to depend on) before fetching a slice; search → show is tackit''s retrieval loop, replacing grep-over-a-monolith. Keyword only; semantic/vector search is explicitly out of scope (a recall optimization with heavy deps). FTS5 is strictly better than grep for this (tokenized, ranked, indexed), but it is only as good as how discoverably tasks are written — see the skill-pack note under "Deferred."

The FTS index carries the synthesized D32 prefix (T161, v0.4): the indexed `name` column is `<kind_letter><id> — <name>` (design→D, schema→S, production→T, meta→M), so `search(''T238'')` and `search(''D23'')` find the right row by id-prefix regardless of whether the stored name carries such a substring. Migration 008 rebuilt the FTS for pre-existing rows.', 'Added the T222 query-sanitization clause to the search contract: input is per-token-quoted so punctuated keys match literally not error; FTS operators not exposed by default; usability not security (MATCH is a bound param).', '2026-06-07T05:50:56.015080+00:00');
INSERT INTO description_revisions (id, task_id, prev_name, prev_description, delta, edited_at) VALUES (64, 57, 'D15 — Task query / board (derived views)', 'List and filter tasks by status, by label, and by stale — e.g. "open tasks in the smoke-test label" or "everything currently stale." The work-queue and any grouping view are queries over the fields, not separately-maintained lists (single source of truth).', 'Added the T220 name_prefix filter axis to the query/board contract: literal case-sensitive prefix on the bare name (not prefixed_name), scopes a query to one section of a large layer; empty prefix refused.', '2026-06-07T05:56:51.409915+00:00');
INSERT INTO description_revisions (id, task_id, prev_name, prev_description, delta, edited_at) VALUES (65, 211, 'Lean read-projection default for ls and board', 'Decision: `ls` and `board` return a LEAN projection by default — task scalars only (id, prefixed_name, name, kind, status, stale, created_at, updated_at, labels), NOT `description`. `board` additionally drops each neighbor''s `because` + `last_edit_delta` by default, keeping the graph SHAPE (NeighborRef id/prefixed_name/status/stale/kind). Full body and full neighbor prose are opt-in.
...
Deferred (triaged, revisit ONLY if lean defaults prove insufficient — not a graveyard backlog): general `fields` whitelist; pagination (limit/offset); total / count-only. Each was considered and set aside because the lean default + show(id) cover the 95% case (one body at a time = show).', 'T221: corrected the lean-set enumeration — the list projection now DROPS the bare name + created_at/updated_at (kept wont_do_reason), and board neighbors drop name too; show(id) is the full-scalar path.', '2026-06-07T06:02:12.142207+00:00');
INSERT INTO description_revisions (id, task_id, prev_name, prev_description, delta, edited_at) VALUES (70, 44, 'D2 — Typed validation boundary', 'Every read and write passes through Pydantic models. Malformed data — a bad status, a missing required field, an edge pointing at a nonexistent task — is rejected at the boundary as a loud error, never stored. This is the "deterministic" guarantee in practice.
Un-storable text is also refused (2026-05-30, closed-issue #22): a stored name/description/label may not contain a NUL byte (it breaks the D18 executescript rebuild from tackit.sql) or an unpaired UTF-16 surrogate (not valid UTF-8, so SQLite can''t encode it). Both are rejected loudly so the store always stays storable and rebuildable. (Both were found by the property-based round-trip test, not by example tests — see Testing.)', 'extend D2 fail-loud boundary to the MCP adapter param surface: reject unknown tool params loudly (additionalProperties:false), not silently', '2026-06-23T06:13:08.190008+00:00');
INSERT INTO description_revisions (id, task_id, prev_name, prev_description, delta, edited_at) VALUES (71, 51, 'D9 — Slice fetch', 'Fetch a single task plus its directly-linked context — its dependencies, dependents, and labels — as one small payload. This is what lets the agent read only what a task needs (a step-sized slice) instead of loading a whole monolithic document. The core anti-context-bloat feature.', 'slice projection collapses the identical dependencies/dependents into a single symmetric links list (consequence of D5); halves neighbour payload', '2026-06-23T06:13:08.448124+00:00');
INSERT INTO description_revisions (id, task_id, prev_name, prev_description, delta, edited_at) VALUES (72, 59, 'D17 — Full-text search', 'Find tasks by keyword over name + description, ranked, via SQLite FTS5 — in-process, no external service or GPU. Returns matching task ids + titles + scores. This is the agent''s entry point for finding tasks (e.g. the ids to depend on) before fetching a slice; search → show is tackit''s retrieval loop, replacing grep-over-a-monolith. Keyword only; semantic/vector search is explicitly out of scope (a recall optimization with heavy deps). FTS5 is strictly better than grep for this (tokenized, ranked, indexed), but it is only as good as how discoverably tasks are written — see the skill-pack note under "Deferred."

The FTS index carries the synthesized D32 prefix (T161, v0.4): the indexed `name` column is `<kind_letter><id> — <name>` (design→D, schema→S, production→T, meta→M), so `search(''T238'')` and `search(''D23'')` find the right row by id-prefix regardless of whether the stored name carries such a substring. Migration 008 rebuilt the FTS for pre-existing rows.

Query sanitization (T222): raw queries are per-token double-quoted before MATCH, so dotted/punctuated keys (section ids like 4.2.4, hyphenated terms, apostrophes) match literally instead of erroring as FTS5 syntax. This makes the keyword-only contract robust — intentional FTS operators (prefix *, AND/OR/NEAR) are NOT exposed by default. The MATCH value is a bound parameter, so the sanitization is a usability measure (natural queries must not error), not a security boundary.', 'correct T222 overclaim + add section-id ROUTING decision (option b): detect section-shaped query, route to anchored name-substring; FTS/S5 untouched', '2026-06-23T06:13:08.676557+00:00');
INSERT INTO description_revisions (id, task_id, prev_name, prev_description, delta, edited_at) VALUES (76, 64, 'D22 — Board view (CLI, human-readable) (added 2026-05-30)', 'tackit board renders the state as a human-readable, dependency-aware board in the terminal — grouped (In Flight / Done), colored (open=accent, stale=alert, done=dim; color only on a TTY), each task showing its labels and its needs→ / unblocks→ edges. Filter MODES cover the common questions: default (everything), --status open ("what''s outstanding," the #1 ask), --label X (a group), --stale (after big changes). --json emits the structured slices. CLI-only (the agent uses the structured ls/stale/show tools); it''s the human''s at-a-glance view, the visual cousin of D16 render. (HTML export was considered and dropped — the CLI board + the agent''s change-summaries cover the need.)', 'board edge display collapses the duplicated needs→/unblocks→ into a single symmetric links list (T237/D9)', '2026-06-23T06:15:47.065345+00:00');
INSERT INTO description_revisions (id, task_id, prev_name, prev_description, delta, edited_at) VALUES (81, 44, 'D2 — Typed validation boundary', 'Every read and write passes through Pydantic models. Malformed data — a bad status, a missing required field, an edge pointing at a nonexistent task — is rejected at the boundary as a loud error, never stored. This is the "deterministic" guarantee in practice.
Un-storable text is also refused (2026-05-30, closed-issue #22): a stored name/description/label may not contain a NUL byte (it breaks the D18 executescript rebuild from tackit.sql) or an unpaired UTF-16 surrogate (not valid UTF-8, so SQLite can''t encode it). Both are rejected loudly so the store always stays storable and rebuildable. (Both were found by the property-based round-trip test, not by example tests — see Testing.)

Adapter param gate (T236, 2026-06-23): the fail-loud boundary extends to the MCP/CLI ADAPTER''s parameter surface, not just core-model data. A tool call carrying an UNRECOGNISED parameter (e.g. ls(query=...) when ls accepts status/label/kind/stale/name_prefix) is REJECTED loudly — the tool input schemas set additionalProperties:false and the error names the offending param plus the valid set — rather than silently dropped. Rationale: a silently-ignored unknown param ran the op UNFILTERED and masked the agent''s mistake (a dogfooding project dogfood: ls(query=...) returned all 525 tasks, read as "the filter is broken"), so the agent never learned the right syntax and resorted to a Python workaround. Loud rejection forces in-session self-correction. Value-level validation was already loud (status=''status:open'' errors helpfully); this closes specifically the unknown-param-NAME hole.', 'privacy: remove private project name from the public spec dump (genericize to ''a dogfooding session'')', '2026-06-23T07:03:50.274872+00:00');
INSERT INTO description_revisions (id, task_id, prev_name, prev_description, delta, edited_at) VALUES (82, 59, 'D17 — Full-text search', 'Find tasks by keyword over name + description, ranked, via SQLite FTS5 — in-process, no external service or GPU. Returns matching task ids + titles + scores. This is the agent''s entry point for finding tasks (e.g. the ids to depend on) before fetching a slice; search → show is tackit''s retrieval loop, replacing grep-over-a-monolith. Keyword only; semantic/vector search is explicitly out of scope (a recall optimization with heavy deps). FTS5 is strictly better than grep for this (tokenized, ranked, indexed), but it is only as good as how discoverably tasks are written — see the skill-pack note under "Deferred."

The FTS index carries the synthesized D32 prefix (T161, v0.4): the indexed `name` column is `<kind_letter><id> — <name>` (design→D, schema→S, production→T, meta→M), so `search(''T238'')` and `search(''D23'')` find the right row by id-prefix regardless of whether the stored name carries such a substring. Migration 008 rebuilt the FTS for pre-existing rows.

Query sanitization (T222): raw queries are per-token double-quoted before MATCH, so dotted/punctuated keys (section ids like 4.2.4, hyphenated terms, apostrophes) match literally instead of erroring as FTS5 syntax. This makes the keyword-only contract robust — intentional FTS operators (prefix *, AND/OR/NEAR) are NOT exposed by default. The MATCH value is a bound parameter, so the sanitization is a usability measure (natural queries must not error), not a security boundary.

Section-id retrieval — route, don''t re-tokenize (T238, 2026-06-23): the "match literally" claim in the T222 paragraph above is too strong. T222 stopped the FTS5 SYNTAX ERROR on dotted keys, but the default unicode61 tokenizer still SHREDS a section id at INDEX time — "§9.4" indexes as the bare tokens 9,4, indistinguishable from "§8.9.4" (8,9,4) — so FTS cannot rank a section lookup correctly (it ranked §8.9.4 ABOVE the literal §9.4 on a dogfooding project''s all-§ corpus). DECISION (option b, chosen over a global tokenizer change because "." is not categorically a section separator, so redefining it for all indexed text is wrong): core.search DETECTS a section-id-shaped query — the whole trimmed query matches ^§?\d+(\.\d+)*$ AND carries a § or a "." (so a bare "11" stays a normal keyword search) — and ROUTES it to an anchored substring match on the raw `name` column instead of FTS. The leading § is the left-anchor that excludes §8.9.4; a right-boundary (the char after the id must be a non-digit) excludes siblings §9.40/§9.41 while including descendants §9.4.x. No schema/migration change — S5''s FTS index is untouched and still serves every non-section query (including the D32 id-prefix lookups).', 'privacy: remove private project name from the public spec dump (genericize to ''a dogfooded corpus'')', '2026-06-23T07:03:50.512810+00:00');
COMMIT;
