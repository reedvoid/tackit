...
def build_server() -> FastMCP:
    ...
    def add(
        ...
        (D5). Returns the new task's slice."""
        with _core() as c:
            t = c.add(name, description=description, labels=labels, deps=deps)
            return c.show(t.id).model_dump(mode="json")

    @mcp.tool()
    def show(id: int) -> dict:
        """Slice fetch (D9): a task plus its dependencies, dependents, and labels."""
        with _core() as c:
            return c.show(id).model_dump(mode="json")

    @mcp.tool()
    def search(terms: str, limit: int = 20) -> list[dict]:
        ...
        with _core() as c:
            return [h.model_dump(mode="json") for h in c.search(terms, limit=limit)]
    ...
    def edit(id: int, name: str | None = None, description: str | None = None) -> dict:
        ...
        with _core() as c:
            return c.edit(id, name=name, description=description).model_dump(mode="json")

    @mcp.tool()
    def close(id: int) -> dict:
        """Close a task (D12). REFUSED if the task is stale (reconcile first, D14).
        On success returns the task's dependencies and dependents to review."""
        with _core() as c:
            return c.close(id).model_dump(mode="json")

    @mcp.tool()
    def reopen(id: int) -> dict:
        """Move a closed task back to open (D7/D8, logged)."""
        with _core() as c:
            return c.reopen(id).model_dump(mode="json")
    ...
    def reconcile(id: int) -> dict:
        ...
        with _core() as c:
            return c.reconcile(id).model_dump(mode="json")
    ...
    def stale() -> list[dict]:
        ...
        with _core() as c:
            return [t.model_dump(mode="json") for t in c.stale_worklist()]
    ...
    @mcp.tool()
    def history(id: int) -> list[dict]:
        """The append-only status-transition history of a task (D8)."""
        with _core() as c:
            return [h.model_dump(mode="json") for h in c.history(id)]
