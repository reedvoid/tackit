...
D256 creation-gate note: every bare `production` add below is wired at
creation to a `design` anchor (`deps={<id>: ...}`) so the creation-gate
passes; tests that fetch "the just-created task" via `core.ls()[0]` now use
`core.ls(kind="production")[0]` since the anchor sorts first by id."""
...
def test_project_task_lean_omits_description(core):
    ...
    core.add(
        "a task", kind="production", description="a long body",
        deps={1: "realizes the anchor decision"},
    )
    t = core.ls(kind="production")[0]
...
def test_project_task_drops_redundant_name_and_timestamps(core):
    ...
    scalars stay."""
    core.add("spec anchor", kind="design")  # D1 -- D256 anchor
    core.add("a task", kind="production", deps={1: "realizes the anchor decision"})
    t = core.ls(kind="production")[0]
    lean = project_task(t, include_description=False)
...
def test_show_remains_the_full_scalar_path(core):
    ...
    core.add("spec anchor", kind="design")  # D1 -- D256 anchor
    core.add("a task", kind="production", deps={1: "realizes the anchor decision"})  # T2
    full = core.show(2).model_dump(mode="json")
...
def test_project_slice_drops_focal_and_neighbor_name(core):
    ...
    t = core.add("impl", kind="production", deps={d.id: "T2 realizes D1"})  # T2
    core.link_add(t.id, d.id, because="T2 realizes D1")  # no-op: link already wired via deps
    card = project_slice(
...
def test_project_task_empty_description_omitted_not_truncated(core):
    core.add("spec anchor", kind="design")  # D1 -- D256 anchor
    core.add("a task", kind="production", deps={1: "realizes the anchor decision"})  # description defaults to ""
    t = core.ls(kind="production")[0]
...
def test_project_slice_lean_drops_body_and_neighbor_because(core):
    d = core.add("design", kind="design")  # D1
    t = core.add(
        "impl", kind="production", description="impl body",
        deps={d.id: "T2 realizes D1"},
    )  # T2
    core.link_add(t.id, d.id, because="T2 realizes D1")  # no-op: link already wired via deps
    lean = project_slice(
        core.show(t.id), include_description=False, include_neighbor_because=False
...
def test_ls_kind_filter_and_composition(core):
    ...
    core.add("p", kind="production", deps={1: "realizes D1"})  # T3 open -- D256 anchor: D1
...
def test_ls_kind_filter_zero_matches(core):
    # D256 anchor is `schema` (not `design`) so it doesn't disturb the
    # "kind=design returns []" assertion this test pins.
    core.add("s", kind="schema")  # S1 -- D256 anchor
    core.add("p", kind="production", deps={1: "realizes S1"})
...
def test_cli_board_lean_default_and_opt_in(cli, capsys):
    ...
    main(["add", "impl", "--kind", "production", "--desc", "impl body",
          "--dep", "1::T2 realizes D1"])  # T2 -- deps wires the link at creation (D256)
    capsys.readouterr()
